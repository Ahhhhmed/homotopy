                        ...
                        with open(os.path.join(item, file_name)) as opened_file:
                            for snippet in filter(language_filter, json.load(opened_file)):
                                if snippet[SnippetProvider.name_key] in self.data:
                                    logging.warning("Multiple definitions for snippet: %s" % snippet[SnippetProvider.name_key])
                                else:
                                    self.data[snippet[SnippetProvider.name_key]] = snippet[SnippetProvider.snippet_key]
                    except (ValueError, OSError):
                        logging.warning("Could not read snippet definitions from file: %s. Ignoring this file." % file_name, exc_info=True)
            except FileNotFoundError:
                logging.warning("Could not open path: %s. Ignoring this folder." % item)
